...
export default function Partner() {
    ...
    useEffect(() => {
        const fetchCards = async () => {
            ...
            // パートナーのカードのみをフィルタリング
            const partnerImages = loadedImages.filter(image => image.type === 'partner');
            const sortedImages = sortImages(partnerImages);
    ...
    //             ...doc.data()
    //         }) as ImageData);
    //         const sortedImages = sortImages(loadedImages);
    //         setImages(sortedImages);
    //         setFilteredImages(sortedImages);
    //     };
    ...
    const handleConfirm = () => {
        if (selectedImage) {
            localStorage.setItem('partner', JSON.stringify(selectedImage));
            setSelectedImage(null);
            router.push('/deck/case-list');
        }
    };
